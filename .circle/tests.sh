...
case ${CIRCLE_NODE_INDEX} in
  0)
    ...
    [[ "$RET" -eq "1" ]]
    # Clean up
    find ~/ds054/scratch -not -name "*.svg" -not -name "*.html" -not -name "*.rst" -not -name "*.mat" -type f -delete
  ...
  1)
    ...
    find ~/ds005/scratch -not -name "*.svg" -not -name "*.html" -not -name "*.rst" -not -name "*.mat" -type f -delete
    ;;
esac
