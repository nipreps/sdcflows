...
class Coefficients2Warp(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        data[np.ones_like(data, dtype="bool")] = (
            np.squeeze(np.vstack(coeffs).T) @ sparse_vstack(weights)
        )
...
def _fix_topup_fieldcoeff(in_coeff, fmap_ref, pe_dir, ro_time=1.0, out_file=None):
