...
"""
    ...
    syn = pe.Node(
        Registration(
            from_file=load_data(f"sd_syn{'_sloppy' * sloppy}.json")
        ),
        name="syn",
...
def init_syn_preprocessing_wf(
    *,
    ...
    transform_list.inputs.in3 = load_data("fmap_atlas_2_MNI152NLin2009cAsym_affine.mat")
    prior2epi = pe.Node(
        ApplyTransforms(
            invert_transform_flags=[True, False, False],
            input_image=load_data("fmap_atlas.nii.gz"),
        ),
        name="prior2epi",
        n_procs=omp_nthreads,
        mem_gb=0.3,
    ...
    epi2anat = pe.Node(
        Registration(from_file=load_data("affine.json")),
        name="epi2anat",
        n_procs=omp_nthreads,
    )
