...
def sort_fmaps(files):
    fmaps = {}
    for fmap_type in fieldmap_suffixes.keys():
        fmaps[fmap_type] = []
        fmaps[fmap_type] = [doc for doc in files if is_fmap_type(fmap_type, doc)]
    # funky return statement so sort_fmaps can be a Function interface
    return (fmaps[key] for key in sorted(fieldmap_suffixes.keys()))
